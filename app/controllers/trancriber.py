...
class TranscriptionThread(QThread):
    ...
    def __init__(self, file_paths, idioma_entrada):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            for index, (file_name, file_path) in enumerate(self.file_paths.items()):
                if not self.transcription_active:
                    break

                # Simulate processing a file (replace with actual transcription logic)
                progress = int(((index + 1) / total_files) * 100)
                self.progress_signal.emit(progress)

                # Placeholder transcription logic
                transcription = f"{file_name}: Transcription example text."
...
class TranscriptionWindow(QMainWindow):
    ...
    def start_transcription(self):
        if self.transcription_thread and self.transcription_thread.isRunning():
            ...
            QMessageBox.information(self, "Transcription", "Transcription stopped.")
            return

        # Collect file paths and language
        file_paths = {"example.wav": "/path/to/example.wav"}  # Replace with actual logic
        idioma_entrada = "en-US"  # Replace with actual selection

        self.transcription_thread = TranscriptionThread(file_paths, idioma_entrada)

        # Connect signals
        self.transcription_thread.progress_signal.connect(self.update_progress_bar)
        self.transcription_thread.result_signal.connect(self.display_results)
    ...
    def display_results(self, result):
        self.ui.textEdit_Transcipcions.append(result)
        QMessageBox.information(self, "Transcription", "Transcription completed.")
...
if __name__ == "__main__":
    import sys
    from ui.form_main import Ui_MainWindow  # Replace with your actual UI import
    ...
    transcription_window = TranscriptionWindow(ui)
    main_window.show()
    sys.exit(app.exec())
